...
// raydium_clmm_pool. Callers wrap the result as one dispatch case of a
// single, consolidated "get top pools" tool (see prefetch/prompttool.go)
// rather than exposing their own separate tool -- keeps the model from
// having to choose between many near-identical per-DEX tools.
func CreateQuery(db *sql.DB) (*Query, error) {
	rqs := new(Query)
	rqs.db = db
	var err error
	// Ranked by liquidity_hi -- same proxy-for-liquidity ordering already
	// used elsewhere in this codebase (cmd/harness.go's example question
	// set, the dashboard's coverage-card sort links), not a full USD
	// valuation. hex(pubkey) so the caller gets a readable
	// base58-decodable string back, not a raw BLOB.
	rqs.topPool, err = db.Prepare("SELECT hex(pubkey) FROM raydium_clmm_pool ORDER BY liquidity_hi DESC LIMIT ?")
	if err != nil {
...
// TopPools returns up to count raydium_clmm_pool pubkeys, ranked by
...
func (q *Query) TopPools(ctx context.Context, count int) (string, error) {
	...
	header := "Raydium CLMM pools, ranked by liquidity_hi (on-chain liquidity proxy) highest first:\n"
