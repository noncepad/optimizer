...
type eventHandler struct {
	...
	errorC         chan<- error
	onTokenResultC chan *clmmVaultResult
	slot           *atomic.Uint64
	logger         *slog.Logger
	mPool          map[sgo.PublicKey]*PoolStateWithToken
}

type clmmVaultResult struct {
	pool   sgo.PublicKey
	av     *sgotkn.Account
...
func createHandler(ctx context.Context, programID sgo.PublicKey, cancel context.CancelCauseFunc, poolMapC chan<- map[sgo.PublicKey]*PoolStateWithToken, errorC chan<- error, logger *slog.Logger) graph.Hook {
	...
	eh.onTokenResultC = make(chan *clmmVaultResult, 100)
...
func loopOnAck(
	...
	mPending := make(map[sgo.PublicKey]*clmmVaultResult)
done:
	for {
		select {
		case <-doneC:
	...
	logger.Info("clmm loopOnAck - 2")
	for k, v := range mPending {
		x, present := mPool[v.pool]
		if present {
			if x.Info.TokenVault0.Equals(k) {
				x.TokenVault0 = v.av
			} else if x.Info.TokenVault1.Equals(k) {
				x.TokenVault1 = v.av
			}
		}
	}
...
func (handler *eventHandler) OnAccount(a graph.Account, isNew bool) {
	...
	if err != nil {
		handler.logger.Warn("clmm PoolState parse failed", "pubkey", header.Pubkey, "err", err)
		return
	}
	...
	for _, vaultPubkey := range []sgo.PublicKey{pool.TokenVault0, pool.TokenVault1} {
		vp := vaultPubkey
		...
		handler.wg.Go(func() {
			...
			if acc != nil {
				...
				if err2 := bin.UnmarshalBorsh(d, acc.Data()); err2 == nil {
					select {
					case <-doneC:
					case tokenResultC <- &clmmVaultResult{pool: poolPubkey, av: d, pubkey: vp}:
					}
				}
			}
		})
