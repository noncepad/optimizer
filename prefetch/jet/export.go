...
// ExportJSON writes every tracked Jet reserve to path as a JSON array of
// Reserve values (its own existing json tags) -- the same real query
...
func ExportJSON(db *sql.DB, path string) error {
	...
	if err != nil {
		return fmt.Errorf("jet: export json: %w", err)
	}
	if reserves == nil {
		reserves = []*Reserve{}
	}
	...
	if err != nil {
		return fmt.Errorf("jet: create %s: %w", path, err)
	}
	defer func() {
		_ = f.Close()
	}()
	...
	if err := enc.Encode(reserves); err != nil {
		return fmt.Errorf("jet: encode %s: %w", path, err)
	}
	return nil
}
