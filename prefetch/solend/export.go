...
// ExportJSON writes every tracked Solend reserve to path as a JSON array
...
func ExportJSON(db *sql.DB, path string) error {
	...
	if err != nil {
		return fmt.Errorf("solend: export json: %w", err)
	}
	if reserves == nil {
		reserves = []*Reserve{}
	}
	...
	if err != nil {
		return fmt.Errorf("solend: create %s: %w", path, err)
	}
	defer func() {
		_ = f.Close()
	}()
	...
	if err := enc.Encode(reserves); err != nil {
		return fmt.Errorf("solend: encode %s: %w", path, err)
	}
	return nil
}
