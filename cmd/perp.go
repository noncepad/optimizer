...
import (
	...
	"git.noncepad.com/pkg/optimizer/brain/perpfundingv1"
...
func (r *PerpCmd) Run(rc *RunConfig) error {
	...
	// PerpCmd only ever reads the prefetch database that DownloadArbCmd
	...
	// the moment the graph hook starts up. perpfundingv1.Create wires up
	// the real implementation: it uploads botImage (the same mode-agnostic
	// wasm binary just compiled above, with MODE=perpfundingv1 selected at
	// upload time) to the validator and drives the handshake.
	b, err := perpfundingv1.Create(ctx, cancel, parentKey, &perpfundingv1.Configuration{
		BotImage: botImage.Path(),
	}, prefetchDB.Raw())
	if err != nil {
		return fmt.Errorf("failed to create perpfundingv1 brain: %s", err)
	}
	mode := "perpfundingv1"
