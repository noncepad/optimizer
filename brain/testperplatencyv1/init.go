...
func (hs *eventHook) Init(g graph.Graph, builder *txbuilder.BuildManager, addressBook common.BotClientDialer, bidmgr *bidder.BidderManager, authorizer sgo.PublicKey) error {
	...
	// test whether that ~42,000-account subscription (unconditional in
	...
	// TEST_PROTOCOL scopes a real run to just one protocol's
	// deposit<->withdraw cycle (see Configuration.TargetProtocol's doc
	// comment) -- read by catscope-rust-bot's
	// TestProtocol::from_env (src/brain/testperplatencyv1/state.rs). Left
	// unset (the Rust side's from_env then returns None) runs the
	// original, unscoped full sequence -- almost never what a real run
	// wants, since it multiplies the real transaction fees spent.
	if 0 < len(hs.config.TargetProtocol) {
		...
		if hs.config.TargetProtocol == "native_lite" {
			mEnv["TEST_PROTOCOL"] = "native"
		} else {
			mEnv["TEST_PROTOCOL"] = hs.config.TargetProtocol
		}
	}
