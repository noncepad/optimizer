...
// process/command, not run from here) down to the bot on lstApyTicker,
// once at startup and every lstApyPushInterval thereafter -- see
// message.go's KeyFlagLstApy doc comment and this bot's own
// log_lst_loop_projection (state.rs) for what the Rust side does with it.
...
// snapshot, matching the example window in lst-yield's own doc comment.
const lstApyEstimateWindow = 7 * 24 * time.Hour

// pushLstApyEstimates reads lstyield.EstimateAPY for every tracked LST and
// pushes whichever ones have a real estimate (not nil -- see EstimateAPY's
// own doc comment on why a fresh watch-lst-yield has nothing to report
// yet) down to the bot as a KeyFlagLstApy message each. One bad estimate
// or send never blocks the others.
func pushLstApyEstimates(instance mgrbot.Bot, db *sql.DB, entry *slog.Logger) {
	for _, lst := range lstyield.KnownLSTs {
		apy, err := lstyield.EstimateAPY(db, lst.Mint, lstApyEstimateWindow)
		if err != nil {
			entry.Error(fmt.Sprintf("lst apy estimate: %s: %s", lst.Symbol, err))
			continue
		}
		if apy == nil {
			continue
		}
		if err := instance.CustomStdin(DoLstApy(lst.Symbol, *apy)); err != nil {
			entry.Error(fmt.Sprintf("lst apy send: %s: %s", lst.Symbol, err))
			continue
		}
		entry.Info(fmt.Sprintf("pushed lst apy estimate: %s=%.3f%%", lst.Symbol, *apy*100))
	}
}
