...
func (hs *eventHook) SendBundlerTipUpdate(update bundler.TipUpdate) error {
	...
	if err := hs.instance.CustomStdin(bundler.DoBundlerTipUpdate(update)); err != nil {
		return fmt.Errorf("testperpv1: send bundler tip update: %w", err)
	}
	return nil
}

// SendTriggerTestAstralane sends a real, one-shot request that proves the
// generic transactionprocessor::batch host import routed to Astralane
// specifically -- see DoTriggerTestAstralane's doc comment.
func (hs *eventHook) SendTriggerTestAstralane() error {
	if hs.instance == nil {
		return ErrBotNotConnectedYet
	}
	if err := hs.instance.CustomStdin(DoTriggerTestAstralane()); err != nil {
		return fmt.Errorf("testperpv1: send trigger test astralane: %w", err)
	}
	return nil
}
...
func (hs *eventHook) initWallet() error {
	...
	// deposit/withdraw transactions with (testperpv1's whole reason to
	// exist) -- subscribe/register the same way arbv1 does so the WASM
	// bot can see its own wallet's token/SOL balances.
	...
	// target and skips itself -- exactly what happened the first time
	// this ran (real balance was nonzero, but nothing was ever sent).
	//
	// parentSOL reading 0 forever was root-caused to two bugs in the
	// shared bot/solpipe/bidder/manager package (not this subscribe call,
	// not depth, not the pubkey) -- see commit.go's CommitFinish (was
	// unconditionally returning true, which tells state.Client.Hook to
	// stop the whole graph subscription after the very first commit) and
	// manager.go's Create (never wrapped mothershipSolpipe through
	// bot/solpipe.New, so OnMarket/OnPipeline/OnSol/etc were never
	// dispatched to at all). Both fixed; see
...
// Solend/Kamino accounts testperpv1's Rust-side evaluate() state machine
// creates (obligation, user_metadata, Kamino's farmer PDA) -- nothing
// else funds it.
...
func (hs *eventHook) Evaluate(solpipeState brain.SolpipeState, bidderState brain.BidderState) error {
	...
	if parentSOL == 0 {
		...
		// exactly like the first real run of this bot did.
		return nil
	}
	hs.didBootTransfer = true
	if parentSOL <= targetParentRemainingLamports {
		hs.logger.Info(fmt.Sprintf(
			"testperpv1: boot transfer skipped -- parent balance %d lamports already at or below the %d lamport target",
	...
	if err != nil {
		cancel()
		return fmt.Errorf("testperpv1: boot transfer: failed to create helper: %s", err)
	}
	...
	go func() {
		...
		if err != nil {
			hs.logger.Info(fmt.Sprintf("testperpv1: boot transfer failed: %s", err))
			return
		}
		hs.logger.Info(fmt.Sprintf(
			"testperpv1: boot transfer sent %d lamports parent->child (leaving ~%d lamports in parent): %s @ slot %d",
...
func (hs *eventHook) SendTargetAllocation(symbol string, allocationPct float64) error {
	if err := perpfunding.SetTargetAllocation(hs.db, symbol, allocationPct); err != nil {
		return fmt.Errorf("testperpv1: persist target allocation: %w", err)
	}
	if err := hs.instance.CustomStdin(DoTargetAllocation(symbol, allocationPct)); err != nil {
		return fmt.Errorf("testperpv1: send target allocation: %w", err)
	}
	return nil
}
